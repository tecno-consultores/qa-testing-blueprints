# Pipelines de CI/CD (Integración Continua)[cite: 20]
...
## 1. El Anti-Patrón vs. La Vía "Blueprints"[cite: 20]
...
## 2. Anatomía de la Ejecución en CI[cite: 20]
...
Lanzar la suite utilizando `docker compose -f docker-compose.qa.yml run --rm`. Si la prueba de cobertura (ej. `kcov` o `pytest-cov`) detecta menos del 95%, arrojará automáticamente un código de salida `1` (Exit Code 1). El sistema de CI intercepta este código y marca el *Pull Request* como "Fallido" (Red/Failed), bloqueando el *merge*.
### Paso 3: Extracción de Artefactos (Reportes)
Como utilizamos volúmenes *bind* (`- .:/app` en nuestro `docker-compose.qa.yml`), cuando el contenedor efímero muere, deja los reportes HTML (cobertura, Playwright, Stryker) físicamente en el disco del *runner*. El paso 3 toma esa carpeta y la sube como un "Artifact" para que el equipo pueda descargarla y ver qué falló.
### Paso 4: Limpieza
...
## 3. Ejemplo Práctico: GitHub Actions[cite: 20]
...
## 4. Consideraciones Avanzadas y Solución de Problemas[cite: 20]
...
```yaml
      - name: Levantar API en segundo plano
        run: docker compose -f docker-compose.qa.yml up -d api-prod
      ...
      - name: Ejecutar BDD (Karate Labs) contra la API viva
        run: docker compose -f docker-compose.qa.yml run --rm karate
```
