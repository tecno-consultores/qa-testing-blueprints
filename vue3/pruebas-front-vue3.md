...
**NUNCA** instruyas al usuario a instalar dependencias de Node localmente ni a usar `docker build`. Toda la ejecución ocurre en contenedores efímeros usando Docker Compose.
...
```bash
docker compose run --rm ui-e2e bash -c "npm install && npm run build"
```
Una vez compilado, levanta el servidor que servirá la UI:
```bash
docker compose up -d ui-prod
```
...
```bash
docker compose run --rm ui-e2e bash -c "npm run type-check && npm run lint"
```

*   **Pruebas Unitarias (Vitest):**
```bash
docker compose run --rm ui-e2e npm run test:unit
```
...
```bash
docker compose run --rm ui-e2e npx playwright test --update-snapshots
```
